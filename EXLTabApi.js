...
function EXLTA_shareId(element){
        return $(element).parents('.EXLResult').find('.EXLResultRecordId').attr('name');
}

function EXLTA_getPNX(recordId){
        var r = $('#'+recordId).get(0);
        if (!r.pnx){
                r.pnx = $.ajax({url: 'display.do',data:{fn: 'display', doc: recordId, showPnx: true},async: false,error:function(){log('pnx retrieval error')},timeout:1000}).responseXML;
        }
        return r.pnx;
}
...
function EXLTA_isbns(recordId){
        var pnx = EXLTA_getPNX(recordId);
		var isbns = new Array();
		$(pnx).find('addata > isbn').each(function() {
